...
def gen_data(args):
    ...
    val_data = datas[args.n_samples_train:args.n_samples_train +
                     args.n_samples_val]
    test_data = datas[args.n_samples_train + args.n_samples_val:]

    train_labels = labels[:args.n_samples_train]
    val_labels = labels[args.n_samples_train:args.n_samples_train +
                        args.n_samples_val]
    test_labels = labels[args.n_samples_train + args.n_samples_val:]

    np.savez(
        os.path.join(
            args.data_path, f'{args.n_blocks}blocks-{args.n_samples_train}_train_2d-easy.npz'),
        n_blocks=args.n_blocks,
        coords=train_data,
        labels=train_labels)
    np.savez(
        os.path.join(
            args.data_path, f'{args.n_blocks}blocks-{args.n_samples_val}_val_2d-easy.npz'),
        n_blocks=args.n_blocks,
        coords=val_data,
        labels=val_labels)
    np.savez(
        os.path.join(
            args.data_path, f'{args.n_blocks}blocks-{args.n_samples_test}_test_2d-easy.npz'),
        n_blocks=args.n_blocks,
        coords=test_data,
        labels=test_labels)
