...
def gen_data(n_blocks=8):
    ...
    vector = np.zeros([n_blocks, n_blocks], dtype=int)
    ...
    print('[debug]: image: {}'.format(img))
    vector[permutation[split_points], n_blocks - 1] = 1
    vector[permutation[-1], n_blocks - 1] = 1
    for i in range(n_blocks - 1):
        if col_idxs[i] == col_idxs[i + 1]:
            vector[permutation[i]][permutation[i + 1]] = 1
    print('[debug]: vector: {}'.format(vector))
    return img, vector


def vector2language(vector: np.ndarray, spliter=', '):
    assert len(vector.shape) == 2 and vector.shape[0] == vector.shape[1]
    print('[debug]: clear objs: {}'.format(np.where(vector[:, -1])))
    language = spliter.join(['clear({})'.format(idx) for idx in (np.where(vector[:, -1])[0])])
    language += '\n'
    print('[debug]: language: {}'.format(language))
    language += spliter.join(['On({}, {})'.format(j, i) for i, j in zip(*np.where(vector[:, :-1]))])
    print('[debug]: language: {}'.format(language))
